...
export function activityScore(lastActiveAt: string, now: Date, halfLifeHours: number): number {
...
export function stakeOverlapScore(seeker: MoneyRange, candidate: MoneyRange): number {
  if (seeker.currency !== candidate.currency) return 0;
...
export function availabilityOverlapScore(
  seeker: TimeWindow[],
  candidate: TimeWindow[],
  now: Date,
): number {
  if (seeker.length === 0 || candidate.length === 0) return 0.5;
  const wd = now.getUTCDay() as TimeWindow["weekday"];
  const seekerToday = seeker.filter((w) => w.weekday === wd);
  const candToday = candidate.filter((w) => w.weekday === wd);
  if (seekerToday.length === 0 || candToday.length === 0) return 0.25;
  ...
  for (const s of seekerToday) {
    ...
    for (const c of candToday) {
      ...
      const overlap = hi - lo;
      if (overlap > 0) best = Math.max(best, overlap / sSpan);
    }
  }
  return clamp01(best);
}

export function skillFitScore(seekerRating: number, candidateRating: number, band: number): number {
  const delta = Math.abs(seekerRating - candidateRating);
  if (band <= 0) return delta === 0 ? 1 : 0;
  return clamp01(1 - delta / band);
}

export function distanceScore(distanceKm: number, maxTravelKm: number): number {
  if (maxTravelKm <= 0) return distanceKm === 0 ? 1 : 0;
  return clamp01(1 - distanceKm / maxTravelKm);
}

export function trustComposite(trust: number, verification: number): number {
  const t = clamp01(trust / 100);
  const v = clamp01(verification / 100);
  return clamp01(0.65 * t + 0.35 * v);
}

export function isPastOpponent(
  seeker: SeekerProfile,
  candidate: Candidate,
  rivalryMode: boolean,
): boolean {
  if (rivalryMode) return false;
...
export function formatCompatible(seeker: SeekerProfile, candidate: Candidate): boolean {
...
export function computeRawScores(input: {
  ...
  if (candidate.sport !== seeker.sport) return null;

  if (seeker.kind === "user" && candidate.party === "user" && candidate.userId === seeker.userId) {
    return null;
  }

  if (!formatCompatible(seeker, candidate)) return null;

  const distanceKm = haversineKm(seeker.location, candidate.location);
  if (distanceKm > seeker.maxTravelDistanceKm) return null;
  ...
  const availabilityScore = availabilityOverlapScore(
    seeker.availabilityUtc,
    candidate.availabilityUtc,
    now,
  );
 ...
 */
export function compositeMatchQuality(raw: RawScores, weights: MatchmakingConfig["weights"]): number {
  const wSum =
    weights.distance +
    weights.skillFit +
    weights.activity +
  ...
  const linear =
    (weights.distance * raw.distanceScore +
 ...
 */
export function fairnessConfidence(raw: RawScores, seekerMaxTravelKm: number): number {
 ...
 */
export function acceptanceLikelihood(
  quality: number,
  raw: RawScores,
): number {
