...
export function buildDeterministicExplanation(input: {
  ...
  parts.push(
    `${fmtKm(raw.distanceKm)} away with strong distance fit (${(raw.distanceScore * 100).toFixed(0)}%).`,
  );

  parts.push(
    `Skill gap is ${raw.skillDelta.toFixed(1)} points vs your configured band; skill fit scores ${(raw.skillFitScore * 100).toFixed(0)}%.`,
  );
  ...
  parts.push(
    `Reliability/trust composite is ${(raw.trustScore * 100).toFixed(0)}% (includes verification).`,
  );
  ...
  parts.push(
    `Overall match quality ${(quality * 100).toFixed(0)}%, fairness confidence ${(fairness * 100).toFixed(0)}%, heuristic acceptance likelihood ${(accept * 100).toFixed(0)}%.`,
  );

  const who =
    candidate.party === "user"
