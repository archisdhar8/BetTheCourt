...
import {
  buildCreditEntry,
  buildDebitAvailableEntry,
  buildEscrowLockEntry,
  buildEscrowPayoutCreditEntry,
  buildEscrowPayoutReleaseEntry,
  buildEscrowRefundEntry,
  buildPlatformFeeCreditEntry,
  projectWalletFromLedger,
} from "./ledger.js";
import { PLATFORM_WALLET_USER_ID, WalletDomainError, type WalletBalances, type WalletProfile } from "./model.js";
import type { LedgerEntry } from "./model.js";
import { newLedgerEntryId, type WalletRepository } from "./repository.js";
...
export class WalletService {
  constructor(
    ...
    private readonly fraud?: FraudService | null,
  ) {}

  async createWallet(input: { userId: string; currency: string }): Promise<WalletProfile> {
    ...
    const w: WalletProfile = { userId: input.userId, currency: input.currency, createdAt: now };
  ...
  async creditWallet(input: {
    ...
    const replay = await this.replayIdempotency<CreditWalletResult>(input.idempotencyKey, CMD_CREDIT);
    ...
    const result: CreditWalletResult = {
  ...
  async debitWallet(input: {
    ...
    const replay = await this.replayIdempotency<DebitWalletResult>(input.idempotencyKey, CMD_DEBIT);
    if (replay) return replay;
    const w = await this.requireWallet(input.userId);
    this.assertCurrency(w.currency, input.currency);
    if (input.amountMinor <= 0) {
    ...
    await this.repo.appendEntries([entry]);
    const result: DebitWalletResult = {
      wallet: await this.getWalletBalances(input.userId),
      ledgerEntryIds: [entry.id],
    };
  ...
  async lockChallengeStake(input: {
    ...
    const replay = await this.replayIdempotency<LockStakeResult>(input.idempotencyKey, CMD_LOCK);
    if (replay) return replay;
    ...
    const side =
      input.userId === ch.creatorPartyId ? "creator" : input.userId === ch.opponentPartyId ? "opponent" : null;
    if (!side) {
      throw new WalletDomainError({
        code: "forbidden",
   ...
   */
  async refundChallengeEscrow(input: { challengeId: string; idempotencyKey: string }): Promise<RefundEscrowResult> {
    const replay = await this.replayIdempotency<RefundEscrowResult>(input.idempotencyKey, CMD_REFUND);
    if (replay) return replay;
    ...
    type RefundTarget = { userId: string; amountMinor: number; currency: string };
    ...
    const result: RefundEscrowResult = {
      wallets,
  ...
  async payoutChallengeEscrow(input: {
    ...
    const replay = await this.replayIdempotency<PayoutEscrowResult>(input.idempotencyKey, CMD_PAYOUT);
    if (replay) return replay;
    ...
    try {
      ...
      const result: PayoutEscrowResult = {
        wallets,
        ledgerEntryIds: entries.map((e) => e.id),
        challenge: updated,
      };
  ...
  async isChallengeFullyEscrowed(challengeId: string): Promise<boolean> {
  ...
  private async ensurePlatformWallet(currency: string): Promise<void> {
  ...
  private async requireWallet(userId: string): Promise<WalletProfile> {
    const w = await this.repo.getWallet(userId);
    if (!w) {
      throw new WalletDomainError({ code: "not_found", message: `Wallet not found for ${userId}`, httpStatus: 404 });
    }
  ...
  private assertCurrency(walletCurrency: string, opCurrency: string): void {
  ...
  private async replayIdempotency<T>(key: string, command: string): Promise<T | null> {
