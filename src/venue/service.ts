...
export const DEFAULT_VENUE_DISCOVERY_CONFIG: VenueDiscoveryConfig = {
  weights: {
    centrality: 0.18,
    travelFairness: 0.22,
...
function mergeConfig(overrides?: PartialVenueDiscoveryConfig): VenueDiscoveryConfig {
  return {
...
function weightedSuitability(
  parts: Record<keyof VenueDiscoveryConfig["weights"], number>,
  weights: VenueDiscoveryConfig["weights"],
): number {
...
function baselineTotalTravelKm(centroid: { lat: number; lng: number }, participants: ParticipantInput[]): number {
...
export async function rankVenues(input: RankVenuesInput): Promise<VenueDiscoveryResult> {
  ...
  for (const venue of input.venues) {
    ...
    if (homeCourt) suitability *= config.homeCourtSuitabilityMultiplier;
