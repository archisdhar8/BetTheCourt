...
export class ChallengeService {
  ...
  private async load(id: string): Promise<Challenge> {
  ...
  private async persist(ch: Challenge): Promise<void> {
  ...
  async submitDraft(id: string, actor: Actor): Promise<Challenge> {
  ...
  async accept(id: string, actor: Actor): Promise<Challenge> {
    const ch = await this.load(id);
    assertNotTerminal(ch);
  ...
  async decline(id: string, actor: Actor): Promise<Challenge> {
    const ch = await this.load(id);
    assertNotTerminal(ch);
  ...
  async cancel(id: string, actor: Actor, reason?: string): Promise<Challenge> {
    const ch = await this.load(id);
    if (isTerminalState(ch.state)) return ch;
  ...
  async patchVenue(id: string, actor: Actor, venueId: string): Promise<Challenge> {
    const ch = await this.load(id);
    ...
    const allowed: ChallengeState[] = ["draft", "pending", "accepted", "funded"];
  ...
  async proposeSchedule(id: string, actor: Actor, proposal: ScheduleProposal): Promise<Challenge> {
    const ch = await this.load(id);
    assertNotTerminal(ch);
   ...
   */
  async applyAgentScheduleConfirmation(
    id: string,
  ...
  async completeMatch(id: string, actor: Actor, result: Record<string, unknown>): Promise<Challenge> {
    const ch = await this.load(id);
    assertNotTerminal(ch);
  ...
  async confirmResult(id: string, actor: Actor, note?: string): Promise<Challenge> {
    const ch = await this.load(id);
    assertNotTerminal(ch);
  ...
  async dispute(id: string, actor: Actor, reason: string): Promise<Challenge> {
    const ch = await this.load(id);
    assertNotTerminal(ch);
  ...
  async resolveDispute(id: string, actor: Actor, resolution: "confirm" | "refund", note?: string): Promise<Challenge> {
    const ch = await this.load(id);
    assertNotTerminal(ch);
    const action: TransitionAction =
      resolution === "confirm" ? "resolve_dispute_confirmed" : "resolve_dispute_refund";
   ...
   */
  async recordPartyFundsLocked(id: string, actor: Actor, side: "creator" | "opponent"): Promise<Challenge> {
   ...
   */
  async finalizePayout(id: string, actor: Actor): Promise<Challenge> {
    if (actor.kind !== "system") {
      throw new ChallengeDomainError({
        code: "forbidden_actor",
  ...
  async assertRankingAllowedForChallenge(id: string): Promise<void> {
    const ch = await this.load(id);
...
export function describeAllowedActions(state: ChallengeState): TransitionAction[] {
