...
import type { ScheduleProposal } from "../challenges/model.js";
import {
...
function assertIsoOrder(startAt: string, endAt: string): void {
...
function buildSlotsFromInput(input: {
  slots: { id?: string; startAt: string; endAt: string; note?: string }[];
  travelBufferMinutes?: number;
  venue?: { lat: number; lng: number };
  creatorLocation?: { lat: number; lng: number };
  opponentLocation?: { lat: number; lng: number };
}): TimeSlot[] {
  ...
  return input.slots.map((s) => {
    ...
    let estimatedMaxTravelKm: number | undefined;
    if (input.venue && input.creatorLocation && input.opponentLocation) {
      estimatedMaxTravelKm = Math.max(
        haversineKm(input.venue, input.creatorLocation),
        haversineKm(input.venue, input.opponentLocation),
      );
    }
    return {
      ...
      effectiveEndAt,
      meta:
        buf || estimatedMaxTravelKm !== undefined
...
function ensurePartyOnChallenge(challenge: { creatorPartyId: string; opponentPartyId: string }, actor: Actor): string {
  if (actor.kind !== "party") {
    throw new SchedulingDomainError({
...
function assertChallengeSchedulable(ch: { state: string }): void {
  if (BLOCKED_SCHEDULE_STATES.includes(ch.state as (typeof BLOCKED_SCHEDULE_STATES)[number])) {
    throw new SchedulingDomainError({
...
export class SchedulingService {
  ...
  async getScheduleView(challengeId: string): Promise<ScheduleView> {
  ...
  async proposeSlots(input: {
    ...
    const rec: ScheduleProposalRecord = {
      id: `spr_${randomUUID()}`,
  ...
  async counterProposeSlots(input: Parameters<SchedulingService["proposeSlots"]>[0]): Promise<ScheduleView> {
    return this.proposeSlots(input);
  }

  async confirmSlot(input: { challengeId: string; actor: Actor; slotId: string }): Promise<ScheduleView> {
    const ch = await this.challenges.getChallenge(input.challengeId);
    if (ch.state !== "funded") {
      throw new SchedulingDomainError({
    ...
    if (cSlot && oSlot && cSlot === oSlot) {
      ...
      await this.repo.saveSession(session);
      const scheduleProposal: ScheduleProposal = toChallengeProposal(chosen);
  ...
  async cancelPendingProposal(input: { challengeId: string; actor: Actor }): Promise<ScheduleView> {
    const ch = await this.challenges.getChallenge(input.challengeId);
    assertChallengeSchedulable(ch);
   ...
   */
  async expireStaleProposals(input: { challengeId: string; asOf?: string }): Promise<ScheduleView> {
  ...
  private async requireSession(challengeId: string): Promise<ScheduleSession> {
...
function emptySession(challengeId: string): ScheduleSession {
  return { challengeId, proposals: [] };
}

function toChallengeProposal(slot: TimeSlot): ScheduleProposal {
  return {
