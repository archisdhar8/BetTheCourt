import { evaluateDeterministicRules, extractWinnerPartyId, samePair, FraudDomainError, } from "./model.js";
const SIX_HOURS_MS = 6 * 60 * 60 * 1000;
function pairMatchesChallenge(ch, s) {
    return samePair(ch.creatorPartyId, ch.opponentPartyId, s.creatorPartyId, s.opponentPartyId);
}
function countPairPriorMatches(ch, snapshots) {
    return snapshots.filter((s) => s.id !== ch.id && pairMatchesChallenge(ch, s)).length;
}
function detectFastRepeat(ch, snapshots) {
    ...
    for (let i = 1; i < times.length; i++) {
        if (times[i] - times[i - 1] <= SIX_HOURS_MS)
            return true;
    }
    return false;
}
function computeSameWinnerStreak(ch, snapshots) {
    const currentWinner = extractWinnerPartyId(ch.result, ch.creatorPartyId, ch.opponentPartyId);
    if (!currentWinner)
        return 0;
    ...
    for (const s of priors) {
        if (s.winnerPartyId !== currentWinner)
            break;
        streak++;
    }
    return streak;
...
export class FraudService {
    ...
    challenges;
    checkin;
    constructor(repo, challenges, checkin) {
        this.repo = repo;
    ...
    async blocksRankingIntegration(challengeId) {
     ...
     */
    async assertPayoutAllowed(challengeId) {
    ...
    async evaluate(input) {
        ...
        const rec = await this.buildEvaluationRecord(input.challengeId, context, prior.length, input.emitPlaceholderSignals === true);
        await this.repo.appendEvaluation(input.challengeId, rec);
        return rec;
    }
    async buildEvaluationRecord(challengeId, context, priorEvaluationCount, emitPlaceholderSignals) {
        ...
        const { fraudScore, signals, recommendedAction, explanation, payoutEligible } = evaluateDeterministicRules(ruleInput);
        return {
            challengeId,
