...
import { createWalletBodySchema, creditWalletBodySchema, debitWalletBodySchema, lockEscrowBodySchema, payoutEscrowBodySchema, refundEscrowBodySchema, } from "../contract.js";
function domainErrorReply(reply, err) {
...
export function registerWalletRoutes(app, wallet) {
    ...
    app.post("/v1/wallets/:userId/credit", async (req, reply) => {
        ...
        try {
            return reply.send(await wallet.creditWallet({
                userId,
    ...
    app.post("/v1/wallets/:userId/debit", async (req, reply) => {
        ...
        try {
            return reply.send(await wallet.debitWallet({
                userId,
                amountMinor: parsed.data.amountMinor,
                currency: parsed.data.currency,
                idempotencyKey: parsed.data.idempotencyKey,
    ...
    app.post("/v1/challenges/:id/escrow/lock", async (req, reply) => {
        ...
        try {
            return reply.send(await wallet.lockChallengeStake({
                challengeId: id,
    ...
    app.post("/v1/challenges/:id/escrow/refund", async (req, reply) => {
        ...
        try {
            return reply.send(await wallet.refundChallengeEscrow({
                challengeId: id,
                idempotencyKey: parsed.data.idempotencyKey,
            }));
        }
    ...
    app.post("/v1/challenges/:id/escrow/payout", async (req, reply) => {
        ...
        try {
            return reply.send(await wallet.payoutChallengeEscrow({
                challengeId: id,
