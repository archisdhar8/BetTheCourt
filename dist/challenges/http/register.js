import { z } from "zod";
import { acceptBodySchema, actorSchema, cancelBodySchema, createChallengeBodySchema, declineBodySchema, patchVenueBodySchema, payoutBodySchema, resolveDisputeBodySchema, } from "../contract.js";
...
export function registerChallengeRoutes(app, challenges) {
    ...
    app.post("/v1/challenges/:id/resolve-dispute", async (req, reply) => {
        ...
        try {
            return reply.send(await challenges.resolveDispute(id, parsed.data.actor, parsed.data.resolution, parsed.data.note));
        }
        catch (err) {
            return domainErrorReply(reply, err);
        }
